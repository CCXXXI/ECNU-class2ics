...
def userLogin():
    ...
    resultDic = {-1: "验证码识别失败，请重试", -2: "用户名或密码错误", -3: "未知错误", 0: "登录成功"}
    loginResult = LogIn.logIn(username, password)
    return makeJsonResponse(
        CODE_SUCCESS if loginResult == 0 else CODE_ERROR,
...
def getClassCSV():
    ...
    if not os.path.isfile(
        DEPLOY_PATH + "/static/temp/json/classInfo_" + requestid + ".json"
    ):
    ...
    csvResult = GetClassCSV.get_class_csv(semester, reminder)
...
def sendFeedback():
    ...
    if len(files) > 0:
        ...
        for one in files:
            one.save(
                os.path.join(
                    DEPLOY_PATH + "static/temp/upload",
                    requestid + "." + secure_filename(one.filename),
                )
            )
